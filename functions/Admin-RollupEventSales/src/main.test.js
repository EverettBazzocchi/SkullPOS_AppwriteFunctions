...
function wireCollections({ events = [], categories = [], ingredients = [], transactions = [] } = {}) {
...
describe("Admin-RollupEventSales", () => {
	...
	test("rolls up sales for a past event and writes them onto the event document", async () => {
		...
		expect(mockDatabases.updateDocument).toHaveBeenCalledWith(
			...
			EVENTS_ID,
			"e1",
			expect.objectContaining({ alcohol_sales: 1000, tips_earned: 100, cash_sales: 1100, revenue: 1100 })
		);
	});
