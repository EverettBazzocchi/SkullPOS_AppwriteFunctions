...
// duplicated in Sales-Report, Transaction-EmailReceipt) -- every
...
// `discount` and exclusive of `tip` -- both are tracked/rolled up as their own separate fields
// (see eventSales.js's own discount_amount/tips_earned) -- so neither needs subtracting here.
//
// The card/stripe leg used to be derived from `payment_due`, which is 0 on every transaction the
// (now-retired) legacy completion path finished, silently understating card revenue in the event
// sales rollup for a real charge. Real legacy rows show `payment_due` reliably held "whatever
